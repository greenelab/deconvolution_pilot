...
input_file = "../../data/celltypist_output/%s_input_data.csv" % sample_id

# Run celltypist
predictions = celltypist.annotate(input_file, majority_voting=True)

# Save output files
label_file = "../../data/celltypist_output/%s_predicted_labels.csv" % sample_id
predictions.predicted_labels.to_csv(label_file, sep="\t")
prob_file = "../../data/celltypist_output/%s_probability_matrix.csv" % sample_id
