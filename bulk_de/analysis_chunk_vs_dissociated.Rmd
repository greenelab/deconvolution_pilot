...
```{r}
gene <- "HBB"
geneCounts <- plotCounts(dds, gene = gene, intgroup = c("sample","condition"),
                         returnData = TRUE)
ggplot(geneCounts, aes(x = condition, y = count, color = sample, group = sample)) +
  scale_y_log10() + geom_point(size = 3) + geom_line() + ggtitle(gene)
